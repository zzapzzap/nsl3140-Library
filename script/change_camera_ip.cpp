...
//   ./change_camera_ip 192.168.2.220 255.255.255.0 192.168.2.1
// Example (specific camera by serial):
//   ./change_camera_ip 192.168.2.220 255.255.255.0 192.168.2.1 N008B060D
...
int main(int argc, char *argv[])
{
    if (argc < 4) {
        ...
        printf("  new_cam_ip   : e.g. 192.168.2.220\n");
    ...
    nsl_setIpAddress(handle, new_ip, netmask, gateway);

    NSL_ERROR_TYPE ret = nsl_saveConfiguration(handle);
    if (ret != NSL_ERROR_TYPE::NSL_SUCCESS) {
        printf("[WARN] nsl_saveConfiguration returned %d — new IP may not survive power cycle.\n", (int)ret);
    } else {
